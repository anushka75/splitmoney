...
@router.get('')
def get_group_members(group_id: int, current_user=Depends(get_current_user), db: Session = Depends(get_db)):
    return get_group_members_controller(db, group_id, current_user)


@router.post('')
def add_group_members(group_id: int, group_member_data: GroupMemberCreate, current_user=Depends(get_current_user), db: Session = Depends(get_db)):
    return add_group_member_controller(db, group_id, current_user, group_member_data)


@router.delete('/{user_id}')
def delete_group_member(group_id: int, user_id: int, current_user=Depends(get_current_user), db: Session = Depends(get_db)):
